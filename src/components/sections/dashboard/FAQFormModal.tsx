...
function buildInitialState(initialData?: FirestoreFAQ): FAQFormState {
  if (!initialData) return { question: "", description: "" };
  return { question: initialData.question, description: initialData.description };
}
...
export const FAQFormModal = ({ mode, initialData, onClose, onSubmit }: FAQFormModalProps) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    onSubmit({ question: values.question.trim(), description: values.description.trim() });
  };
  ...
  return (
    <div
      ...
      role="dialog" aria-modal="true" aria-labelledby="faq-form-title"
      onClick={onClose}
    >
      <div onClick={(e) => e.stopPropagation()} className={`w-full max-w-2xl rounded-2xl border shadow-xl ${isDark ? "bg-bg-dark-1 border-bg-gray-1" : "bg-white border-gray-200"}`}>
        ...
        <form ref={formRef} onSubmit={handleSubmit} noValidate
          ...
          <div>
            ...
            <input id="ff-question" type="text" placeholder="Add question"
          ...
          <div>
            ...
            <textarea id="ff-description" rows={4} placeholder="Answer to the question"
        ...
        <div className={`flex flex-col-reverse sm:flex-row sm:justify-end gap-3 px-6 sm:px-8 py-5 border-t ${isDark ? "border-bg-gray-1" : "border-gray-200"}`}>
          ...
          <Button text={mode === "add" ? "Add FAQ" : "Save Changes"} variant="primary" type="submit"
