...
function validateAll(values: AchievementFormState): Errors {
  const errors: Errors = {};
...
function buildInitialState(initialData?: FirestoreAchievement): AchievementFormState {
  if (!initialData) return { title: "", description: "" };
...
export const AchievementFormModal = ({ mode, initialData, onClose, onSubmit }: AchievementFormModalProps) => {
  ...
  return (
    <div
      ...
      role="dialog" aria-modal="true" aria-labelledby="achievement-form-title"
      onClick={onClose}
    >
      <div onClick={(e) => e.stopPropagation()} className={`w-full max-w-2xl rounded-2xl border shadow-xl ${isDark ? "bg-bg-dark-1 border-bg-gray-1" : "bg-white border-gray-200"}`}>
        ...
        <form ref={formRef} onSubmit={handleSubmit} noValidate
          ...
          <div>
            ...
            <input id="af-title" type="text" placeholder="e.g. 3+ Years of Excellence"
              value={values.title}
          ...
          <div>
            ...
            <textarea id="af-description" rows={4} placeholder="Description"
        ...
        <div className={`flex flex-col-reverse sm:flex-row sm:justify-end gap-3 px-6 sm:px-8 py-5 border-t ${isDark ? "border-bg-gray-1" : "border-gray-200"}`}>
          ...
          <Button text={mode === "add" ? "Add Achievement" : "Save Changes"} variant="primary" type="submit"
