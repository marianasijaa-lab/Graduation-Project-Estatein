...
interface EPMCardFormState {
  title: string;
  description: string;
  icon: string;
}

type F = keyof EPMCardFormState;
...
function validateAll(values: EPMCardFormState): Errors {
  const errors: Errors = {};
...
function buildInitialState(initialData?: FirestoreEffortlessPropertyManagementCard): EPMCardFormState {
...
export const EffortlessPropertyManagementFormModal = ({
  mode, initialData, onClose, onSubmit,
}: EffortlessPropertyManagementFormModalProps) => {
  const { theme } = useTheme();
  const isDark = theme === "dark";

  const [values,  setValues]  = useState<EPMCardFormState>(() => buildInitialState(initialData));
  ...
  return (
    <div
      ...
      role="dialog" aria-modal="true" aria-labelledby="epm-card-form-title"
      onClick={onClose}
    >
      <div onClick={(e) => e.stopPropagation()} className={`w-full max-w-2xl rounded-2xl border shadow-xl ${isDark ? "bg-bg-dark-1 border-bg-gray-1" : "bg-white border-gray-200"}`}>
        ...
        <form ref={formRef} onSubmit={handleSubmit} noValidate
          ...
          <div>
            ...
            <input id="epm-card-title" type="text" placeholder="Add title"
              value={values.title}
          ...
          <div>
            ...
            <textarea id="epm-card-description" rows={4} placeholder="Description"
