...
function validateAll(values: SmartInvestmentsCardFormState): Errors {
  const errors: Errors = {};
...
export const SmartInvestmentsFormModal = ({ mode, initialData, onClose, onSubmit }: SmartInvestmentsFormModalProps) => {
  ...
  return (
    <div
      ...
      role="dialog" aria-modal="true" aria-labelledby="si-card-form-title"
      onClick={onClose}
    >
      <div onClick={(e) => e.stopPropagation()} className={`w-full max-w-2xl rounded-2xl border shadow-xl ${isDark ? "bg-bg-dark-1 border-bg-gray-1" : "bg-white border-gray-200"}`}>
        ...
        <form ref={formRef} onSubmit={handleSubmit} noValidate
          ...
          <div>
            ...
            <input id="si-card-title" type="text" placeholder="Add title"
              value={values.title}
          ...
          <div>
            ...
            <textarea id="si-card-description" rows={4} placeholder="Description"
        ...
        <div className={`flex flex-col-reverse sm:flex-row sm:justify-end gap-3 px-6 sm:px-8 py-5 border-t ${isDark ? "border-bg-gray-1" : "border-gray-200"}`}>
          ...
          <Button text={mode === "add" ? "Add Card" : "Save Changes"} variant="primary" type="submit"
