...
interface SmartInvestmentsCardFormState {
  title: string;
  description: string;
  icon: string;
}

type SmartInvestmentsCardFormErrors = Partial<Record<keyof SmartInvestmentsCardFormState, string>>;
...
function validate(values: SmartInvestmentsCardFormState): SmartInvestmentsCardFormErrors {
  const errors: SmartInvestmentsCardFormErrors = {};
...
interface SmartInvestmentsFormModalProps {
  mode: "add" | "edit";
  initialData?: FirestoreSmartInvestmentsCard;
  onClose: () => void;
  onSubmit: (values: Omit<FirestoreSmartInvestmentsCard, "id">) => void;
}

export const SmartInvestmentsFormModal = ({
  mode,
  initialData,
  onClose,
  onSubmit,
}: SmartInvestmentsFormModalProps) => {
  const { theme } = useTheme();
  const isDark = theme === "dark";

  const [values, setValues] = useState<SmartInvestmentsCardFormState>(() => buildInitialState(initialData));
  const [errors, setErrors] = useState<SmartInvestmentsCardFormErrors>({});
  const formRef = useRef<HTMLFormElement>(null);

  const setField = <K extends keyof SmartInvestmentsCardFormState>(
    field: K,
    value: SmartInvestmentsCardFormState[K],
  ) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload: Omit<FirestoreSmartInvestmentsCard, "id"> = {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3 id="si-card-form-title" className={`text-lg font-semibold ${isDark ? "text-white" : "text-gray-900"}`}>
