  ...
  const handleFormSubmit = async (values: Omit<FirestoreSmartInvestmentsCard, "id">) => {
    try {
      if (formModal?.mode === "edit") {
        ...
        notifySuccess("Investment card updated");
      } else {
        await addDocument<FirestoreSmartInvestmentsCard>("smartInvestments", values);
        notifySuccess("Investment card added");
      }
    } catch (error) {
  ...
  const confirmDelete = async () => {
    if (deleteTarget) {
      try {
        ...
        notifySuccess("Investment card deleted");
      } catch (error) {
                ...
                <motion.tr key={card.id} variants={tableRowVariants}
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <motion.a href="/services#smart-investments" onClick={(e) => e.stopPropagation()} aria-label="View on site" title="View on site" {...iconBtnHover} className={iconBtnClass}>
