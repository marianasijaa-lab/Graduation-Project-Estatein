...
import { SmartInvestmentsFormModal } from "../../components/sections/dashboard/SmartInvestmentsFormModal";
...
type FormModalState = { mode: "add" } | { mode: "edit"; card: FirestoreSmartInvestmentsCard } | null;

function buildCardDetailFields(card: FirestoreSmartInvestmentsCard): DetailField[] {
  return [
...
export const SmartInvestmentsManagement = () => {
  const { theme } = useTheme();
  const isDark = theme === "dark";

  const { smartInvestments, status } = useSmartInvestments();
  const [searchTerm, setSearchTerm] = useState("");
  const [formModal, setFormModal] = useState<FormModalState>(null);
  const [deleteTarget, setDeleteTarget] = useState<FirestoreSmartInvestmentsCard | null>(null);
  const [detailTarget, setDetailTarget] = useState<FirestoreSmartInvestmentsCard | null>(null);

  const filteredCards = smartInvestments.filter((c) => c.title.toLowerCase().includes(searchTerm.trim().toLowerCase()));

  const openAddModal = () => setFormModal({ mode: "add" });
  const openEditModal = (c: FirestoreSmartInvestmentsCard) => setFormModal({ mode: "edit", card: c });
  const closeFormModal = () => setFormModal(null);

  const handleFormSubmit = async (values: Omit<FirestoreSmartInvestmentsCard, "id">) => {
    try {
      if (formModal?.mode === "edit") await updateDocument<FirestoreSmartInvestmentsCard>("smartInvestments", formModal.card.id, values);
      else await addDocument<FirestoreSmartInvestmentsCard>("smartInvestments", values);
  ...
  const confirmDelete = async () => {
    if (deleteTarget) {
      try { await deleteDocument("smartInvestments", deleteTarget.id); }
      catch (error) { console.error("Failed to delete card:", error); }
    }
    setDeleteTarget(null);
  };

  const openRowDetail = (c: FirestoreSmartInvestmentsCard) => setDetailTarget(c);
  const handleRowKeyDown = (e: React.KeyboardEvent, c: FirestoreSmartInvestmentsCard) => {
    ...
    <DashboardPageShell>
      <motion.div variants={staggerItem} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <p className={`text-sm ${isDark ? "text-gray" : "text-gray-500"}`}>
          Manage the cards in the "Smart Investments" section on the Services page — add, update, or remove them. The section's header text and banner stay fixed.
        </p>
      ...
      {status === "loading" && smartInvestments.length === 0 && (
          ...
          <table className="w-full text-sm">
            ...
            <motion.tbody variants={rowStagger} initial="hidden" animate="visible">
              {filteredCards.map((card) => (
                <motion.tr key={card.id} variants={rowVariants}
                  ...
                  <td className={`px-5 py-3 ${isDark ? "text-gray" : "text-gray-600"}`}>#{card.id}</td>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button type="button" onClick={(e) => { e.stopPropagation(); openEditModal(card); }} aria-label={`Edit ${card.title}`}
                      ...
                      <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget(card); }} aria-label={`Delete ${card.title}`}
      ...
      {smartInvestments.length > 0 && (
        <motion.div variants={rowStagger} initial="hidden" animate="visible" className="lg:hidden flex flex-col gap-4">
          {filteredCards.map((card) => (
            <motion.div key={card.id} variants={rowVariants}
              tabIndex={0} role="button" aria-label={`View details for ${card.title}`}
              onClick={() => openRowDetail(card)} onKeyDown={(e) => handleRowKeyDown(e, card)}
              ...
              <div className={`mt-4 grid grid-cols-2 gap-2 pt-4 border-t ${isDark ? "border-bg-gray-1" : "border-gray-200"}`}>
                <button type="button" onClick={(e) => { e.stopPropagation(); openEditModal(card); }}
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget(card); }}
      ...
      {formModal && <SmartInvestmentsFormModal mode={formModal.mode} initialData={formModal.mode === "edit" ? formModal.card : undefined} onClose={closeFormModal} onSubmit={handleFormSubmit} />}
...
export default SmartInvestmentsManagement;
