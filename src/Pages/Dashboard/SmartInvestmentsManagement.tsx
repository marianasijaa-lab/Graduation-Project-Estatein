...
// Dashboard page: list/search/add/edit/delete cards for the Smart Investments
...
export const SmartInvestmentsManagement = () => {
  ...
  const filteredCards = smartInvestments.filter((card) =>
    card.title.toLowerCase().includes(searchTerm.trim().toLowerCase()),
  );

  const openAddModal = () => setFormModal({ mode: "add" });
  const openEditModal = (card: FirestoreSmartInvestmentsCard) => setFormModal({ mode: "edit", card });
  ...
  const handleFormSubmit = async (values: Omit<FirestoreSmartInvestmentsCard, "id">) => {
    try {
      if (formModal?.mode === "edit") {
        ...
        await updateDocument<FirestoreSmartInvestmentsCard>("smartInvestments", formModal.card.id, values);
      } else {
        await addDocument<FirestoreSmartInvestmentsCard>("smartInvestments", values);
      }
    } catch (error) {
  ...
  const confirmDelete = async () => {
    if (deleteTarget) {
      try {
        await deleteDocument("smartInvestments", deleteTarget.id);
      } catch (error) {
  ...
  const openRowDetail = (card: FirestoreSmartInvestmentsCard) => setDetailTarget(card);
  const handleRowKeyDown = (e: React.KeyboardEvent, card: FirestoreSmartInvestmentsCard) => {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <p className={`text-sm ${isDark ? "text-gray" : "text-gray-500"}`}>
          Manage the cards in the "Smart Investments" section on the Services page — add,
          update, or remove them. The section's header text and banner stay fixed.
        </p>
      ...
      {formModal && (
        <SmartInvestmentsFormModal
          mode={formModal.mode}
          initialData={formModal.mode === "edit" ? formModal.card : undefined}
      ...
      {detailTarget && (
        <DetailModal
          ...
          fields={buildCardDetailFields(detailTarget)}
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget
            ? `"${deleteTarget.title}" will be permanently removed from the Services page. This can't be undone.`
            : ""
        }
        confirmLabel="Delete"
