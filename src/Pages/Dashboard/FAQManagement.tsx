    ...
    <DashboardPageShell>
          ...
          <table className="w-full text-sm">
            ...
            <motion.tbody variants={rowStagger} initial="hidden" animate="visible">
              {filteredFAQs.map((faq) => (
                <motion.tr key={faq.id} variants={rowVariants}
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button type="button" onClick={(e) => { e.stopPropagation(); openEditModal(faq); }} aria-label={`Edit ${faq.question}`}
                      ...
                      <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget(faq); }} aria-label={`Delete ${faq.question}`}
      ...
      {faqs.length > 0 && (
        <motion.div variants={rowStagger} initial="hidden" animate="visible" className="lg:hidden flex flex-col gap-4">
          {filteredFAQs.map((faq) => (
            <motion.div key={faq.id} variants={rowVariants}
              tabIndex={0} role="button" aria-label={`View details for ${faq.question}`}
              onClick={() => openRowDetail(faq)} onKeyDown={(e) => handleRowKeyDown(e, faq)}
              ...
              <div className={`mt-4 grid grid-cols-2 gap-2 pt-4 border-t ${isDark ? "border-bg-gray-1" : "border-gray-200"}`}>
                <button type="button" onClick={(e) => { e.stopPropagation(); openEditModal(faq); }}
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget(faq); }}
