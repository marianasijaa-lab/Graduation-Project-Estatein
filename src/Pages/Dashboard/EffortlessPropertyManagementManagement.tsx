...
type FormModalState =
  | { mode: "add" }
  | { mode: "edit"; card: FirestoreEffortlessPropertyManagementCard }
  | null;
...
// Dashboard page: list/search/add/edit/delete cards for the Effortless Property
// Management section on the Services page.
export const EffortlessPropertyManagementManagement = () => {
  ...
  const filteredCards = effortlessPropertyManagement.filter((card) =>
    card.title.toLowerCase().includes(searchTerm.trim().toLowerCase()),
  );

  const openAddModal = () => setFormModal({ mode: "add" });
  const openEditModal = (card: FirestoreEffortlessPropertyManagementCard) => setFormModal({ mode: "edit", card });
  ...
  const handleFormSubmit = async (values: Omit<FirestoreEffortlessPropertyManagementCard, "id">) => {
    try {
      if (formModal?.mode === "edit") {
        ...
        await updateDocument<FirestoreEffortlessPropertyManagementCard>(
  ...
  const openRowDetail = (card: FirestoreEffortlessPropertyManagementCard) => setDetailTarget(card);
  const handleRowKeyDown = (e: React.KeyboardEvent, card: FirestoreEffortlessPropertyManagementCard) => {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <p className={`text-sm ${isDark ? "text-gray" : "text-gray-500"}`}>
          Manage the cards in the "Effortless Property Management" section on the Services
          page — add, update, or remove them. The section's header text and banner stay fixed.
        </p>
      ...
      {status === "loading" && effortlessPropertyManagement.length === 0 && (
      ...
      {/* ── Desktop table ── */}
      {effortlessPropertyManagement.length > 0 && (
      ...
      {formModal && (
        <EffortlessPropertyManagementFormModal
          mode={formModal.mode}
