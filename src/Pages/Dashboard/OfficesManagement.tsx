...
// Every Office field, for the detail view.
...
// Dashboard page: list/search/add/edit/delete company offices.
export const OfficesManagement = () => {
  ...
  const typeOptions = useMemo(
    () => [ALL_TYPES, ...Array.from(new Set(offices.map((o) => o.type))).sort()],
    [offices],
  );
    ...
    return offices
      .filter((office) => {
        const matchesSearch =
          ...
          office.country.toLowerCase().includes(term);
        const matchesType = typeFilter === ALL_TYPES || office.type === typeFilter;
  ...
  const openEditModal = (office: FirestoreOffice) => setFormModal({ mode: "edit", office });
  ...
  const handleRowKeyDown = (e: React.KeyboardEvent, office: FirestoreOffice) => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      openRowDetail(office);
    }
  };
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <p className={`text-sm ${isDark ? "text-gray" : "text-gray-500"}`}>
          Manage the offices shown in the "Discover Our Office Locations" tabs on the Contact
          page — add new locations, update details, or remove closed offices.
        </p>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            aria-label="Search by name, city or country"
      ...
      {status === "failed" && (
        <div className="rounded-2xl border border-rose-500/30 bg-rose-500/10 py-16 text-center text-sm text-rose-500">
          Couldn't load offices. Please try again.
        </div>
      )}
            ...
            <tbody>
              {filteredOffices.map((office) => (
                <tr
                  ...
                  onClick={() => openRowDetail(office)}
                  onKeyDown={(e) => handleRowKeyDown(e, office)}
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <span
                  className={`shrink-0 px-2.5 py-1 rounded-full text-xs font-medium border ${
                    isDark ? "border-bg-gray-1 text-gray" : "border-gray-200 text-gray-600"
                  }`}
                >
                  {office.type}
                </span>
      ...
      {formModal && (
        <OfficeFormModal
          mode={formModal.mode}
          initialData={formModal.mode === "edit" ? formModal.office : undefined}
      ...
      {detailTarget && (
        <DetailModal
          ...
          fields={buildOfficeDetailFields(detailTarget)}
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget
            ? `"${deleteTarget.name}" will be permanently removed from the Contact page. This can't be undone.`
            : ""
        }
        confirmLabel="Delete"
