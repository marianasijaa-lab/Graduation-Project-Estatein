    ...
    <DashboardPageShell>
      ...
      {status === "succeeded" && (
        <motion.div
          ...
          className={`hidden lg:block rounded-2xl border overflow-x-auto table-scroll ${panelClass}`}
        >
          <table className="w-full text-sm">
            <thead>
            ...
            <AnimatePresence mode="wait">
              <motion.tbody
                key={status + filteredProperties.map(p => p.id).join(",")}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
              ...
              {filteredProperties.map((property) => (
                <motion.tr key={property.id} variants={tableRowVariants}
                  ...
                  <td className={`px-5 py-3 ${isDark ? "text-gray" : "text-gray-600"}`}><span title={`Full ID: ${property.id}`} className="cursor-help font-mono">#{shortId(property.id)}</span></td>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <motion.button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget(property); }}
                        aria-label={`Delete ${property.name}`} {...deleteBtnHover}
      ...
      {status === "succeeded" && (
        <motion.div variants={rowStagger} initial="hidden" animate="visible" className="lg:hidden flex flex-col gap-4">
          {filteredProperties.map((property) => (
            <motion.div key={property.id} variants={rowVariants} {...cardHoverProps}
          ...
          {filteredProperties.length === 0 && (
            <motion.div variants={staggerItem} className={`rounded-2xl border py-12 text-center text-sm ${panelClass} ${isDark ? "text-gray" : "text-gray-500"}`}>No properties match your search.</motion.div>
          )}
        </motion.div>
      )}
