...
// Wraps a value so commas / quotes / newlines don't break the CSV.
...
// Dashboard page: review newsletter subscribers, toggle status, export, delete.
  ...
  const filteredSubscribers = useMemo(() => {
    ...
    return subscribers
      .filter((subscriber) => {
        ...
        const matchesStatus = statusFilter === ALL_STATUSES || subscriber.status === statusFilter;
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <p className={`text-sm ${isDark ? "text-gray" : "text-gray-500"}`}>
          People who signed up for Estatein updates through the footer or the Contact page.
          Toggle a subscription, export the list, or remove an address.
        </p>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select
          ...
          className={`${inputClass} px-4 py-3 cursor-pointer sm:w-56`}
        >
              ...
              {filteredSubscribers.map((subscriber) => (
                ...
                >
                  ...
                  <td className={`px-5 py-3 whitespace-nowrap ${isDark ? "text-gray" : "text-gray-600"}`}>
                    {formatCreatedAt(subscriber.createdAt)}
                  </td>
                  <td className="px-5 py-3">
                    <select
                      value={subscriber.status}
                      onChange={(e) => handleStatusChange(subscriber, e.target.value as SubscriberStatus)}
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className={`text-sm truncate ${isDark ? "text-gray" : "text-gray-500"}`}>
                    {subscriber.name || "—"} · {subscriber.source}
                  </p>
                </div>
                <span className={`shrink-0 text-xs ${isDark ? "text-gray" : "text-gray-400"}`}>
                  {formatCreatedAt(subscriber.createdAt)}
                </span>
              </div>
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget
            ? `"${deleteTarget.email}" will be permanently removed from the mailing list. This can't be undone.`
            : ""
        }
        confirmLabel="Delete"
