    ...
    <DashboardPageShell>
      ...
      <motion.div variants={staggerItem} className="relative">
        ...
        <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
          placeholder="Search by client name" aria-label="Search by client name"
          className={`${inputClass} pl-11 pr-4 py-3`} />
      </motion.div>

      {status === "loading" && testimonials.length === 0 && (
        <motion.div variants={staggerItem} className={`rounded-2xl border py-16 text-center text-sm ${panelClass} ${isDark ? "text-gray" : "text-gray-500"}`}>Loading testimonials…</motion.div>
      )}
      {status === "failed" && (
          ...
          <table className="w-full text-sm">
            ...
            <motion.tbody variants={rowStagger} initial="hidden" animate="visible">
              {filteredTestimonials.map((testimonial) => (
                <motion.tr key={testimonial.id} variants={rowVariants}
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button type="button" onClick={(e) => { e.stopPropagation(); openEditModal(testimonial); }} aria-label={`Edit ${testimonial.clientName}`}
                      ...
                      <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget(testimonial); }} aria-label={`Delete ${testimonial.clientName}`}
      ...
      {testimonials.length > 0 && (
        <motion.div variants={rowStagger} initial="hidden" animate="visible" className="lg:hidden flex flex-col gap-4">
          {filteredTestimonials.map((testimonial) => (
            <motion.div key={testimonial.id} variants={rowVariants}
              ...
              <div className={`mt-4 grid grid-cols-2 gap-2 pt-4 border-t ${isDark ? "border-bg-gray-1" : "border-gray-200"}`}>
                <button type="button" onClick={(e) => { e.stopPropagation(); openEditModal(testimonial); }}
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget(testimonial); }}
