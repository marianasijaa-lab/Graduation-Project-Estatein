    ...
    <DashboardPageShell>
          ...
          <table className="w-full text-sm">
            ...
            <motion.tbody variants={rowStagger} initial="hidden" animate="visible">
              {filteredValues.map((value) => (
                <motion.tr key={value.id} variants={rowVariants}
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button type="button" onClick={(e) => { e.stopPropagation(); openEditModal(value); }} aria-label={`Edit ${value.title}`}
                      ...
                      <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget(value); }} aria-label={`Delete ${value.title}`}
      ...
      {values.length > 0 && (
        <motion.div variants={rowStagger} initial="hidden" animate="visible" className="lg:hidden flex flex-col gap-4">
          {filteredValues.map((value) => (
            <motion.div key={value.id} variants={rowVariants}
              tabIndex={0} role="button" aria-label={`View details for ${value.title}`}
              onClick={() => openRowDetail(value)} onKeyDown={(e) => handleRowKeyDown(e, value)}
              ...
              <div className={`mt-4 grid grid-cols-2 gap-2 pt-4 border-t ${isDark ? "border-bg-gray-1" : "border-gray-200"}`}>
                <button type="button" onClick={(e) => { e.stopPropagation(); openEditModal(value); }}
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget(value); }}
