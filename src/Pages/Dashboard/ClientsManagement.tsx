    ...
    <DashboardPageShell>
                ...
                <motion.tr key={client.id} variants={rowVariants}
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button type="button" onClick={(e) => { e.stopPropagation(); openEditModal(client); }} aria-label={`Edit ${client.heading}`}
                      ...
                      <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget(client); }} aria-label={`Delete ${client.heading}`}
      ...
      {clients.length > 0 && (
        <motion.div variants={rowStagger} initial="hidden" animate="visible" className="lg:hidden flex flex-col gap-4">
          {filteredClients.map((client) => (
            <motion.div key={client.id} variants={rowVariants}
              tabIndex={0} role="button" aria-label={`View details for ${client.heading}`}
              onClick={() => openRowDetail(client)} onKeyDown={(e) => handleRowKeyDown(e, client)}
              ...
              <div className={`mt-4 grid grid-cols-2 gap-2 pt-4 border-t ${isDark ? "border-bg-gray-1" : "border-gray-200"}`}>
                <button type="button" onClick={(e) => { e.stopPropagation(); openEditModal(client); }}
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget(client); }}
