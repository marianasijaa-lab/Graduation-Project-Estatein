  ...
  return (
    <DashboardPageShell>
      ...
      {status === "loading" && achievements.length === 0 && (
        <motion.div variants={staggerItem} className={`rounded-2xl border py-16 text-center text-sm ${panelClass} ${isDark ? "text-gray" : "text-gray-500"}`}>
          ...
          <table className="w-full text-sm">
            ...
            <motion.tbody variants={rowStagger} initial="hidden" animate="visible">
              {filteredAchievements.map((achievement) => (
                <motion.tr key={achievement.id} variants={rowVariants}
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button type="button" onClick={(e) => { e.stopPropagation(); openEditModal(achievement); }} aria-label={`Edit ${achievement.title}`}
                      ...
                      <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget(achievement); }} aria-label={`Delete ${achievement.title}`}
      ...
      {achievements.length > 0 && (
        <motion.div variants={rowStagger} initial="hidden" animate="visible" className="lg:hidden flex flex-col gap-4">
          {filteredAchievements.map((achievement) => (
            <motion.div key={achievement.id} variants={rowVariants}
              ...
              <div className={`mt-4 grid grid-cols-2 gap-2 pt-4 border-t ${isDark ? "border-bg-gray-1" : "border-gray-200"}`}>
                <button type="button" onClick={(e) => { e.stopPropagation(); openEditModal(achievement); }}
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteTarget(achievement); }}
