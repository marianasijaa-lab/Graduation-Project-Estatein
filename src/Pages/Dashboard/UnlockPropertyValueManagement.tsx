...
type FormModalState = { mode: "add" } | { mode: "edit"; card: FirestoreUnlockPropertyValueCard } | null;

function buildCardDetailFields(card: FirestoreUnlockPropertyValueCard): DetailField[] {
  return [
...
export const UnlockPropertyValueManagement = () => {
  const { theme } = useTheme();
  const isDark = theme === "dark";

  const { unlockPropertyValue, status } = useUnlockPropertyValue();
  const [searchTerm, setSearchTerm] = useState("");
  const [formModal, setFormModal] = useState<FormModalState>(null);
  const [deleteTarget, setDeleteTarget] = useState<FirestoreUnlockPropertyValueCard | null>(null);
  const [detailTarget, setDetailTarget] = useState<FirestoreUnlockPropertyValueCard | null>(null);

  const filteredCards = unlockPropertyValue.filter((c) => c.title.toLowerCase().includes(searchTerm.trim().toLowerCase()));

  const openAddModal = () => setFormModal({ mode: "add" });
  const openEditModal = (c: FirestoreUnlockPropertyValueCard) => setFormModal({ mode: "edit", card: c });
  ...
  const handleRowKeyDown = (e: React.KeyboardEvent, c: FirestoreUnlockPropertyValueCard) => {
    ...
    <DashboardPageShell>
      <motion.div variants={staggerItem} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <p className={`text-sm ${isDark ? "text-gray" : "text-gray-500"}`}>
          Manage the cards in the "Unlock Property Value" section on the Services page — add, update, or remove them. The section's header text and banner stay fixed.
        </p>
      ...
      {status === "loading" && unlockPropertyValue.length === 0 && (
      ...
      {formModal && <UnlockPropertyValueFormModal mode={formModal.mode} initialData={formModal.mode === "edit" ? formModal.card : undefined} onClose={closeFormModal} onSubmit={handleFormSubmit} />}
...
export default UnlockPropertyValueManagement;
