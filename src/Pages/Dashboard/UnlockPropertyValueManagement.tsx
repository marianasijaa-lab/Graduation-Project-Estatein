...
// Dashboard page: list/search/add/edit/delete cards for the Unlock Property
// Value section on the Services page.
export const UnlockPropertyValueManagement = () => {
  ...
  const filteredCards = unlockPropertyValue.filter((card) =>
    card.title.toLowerCase().includes(searchTerm.trim().toLowerCase()),
  );

  const openAddModal = () => setFormModal({ mode: "add" });
  const openEditModal = (card: FirestoreUnlockPropertyValueCard) => setFormModal({ mode: "edit", card });
  ...
  const openRowDetail = (card: FirestoreUnlockPropertyValueCard) => setDetailTarget(card);
  const handleRowKeyDown = (e: React.KeyboardEvent, card: FirestoreUnlockPropertyValueCard) => {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <p className={`text-sm ${isDark ? "text-gray" : "text-gray-500"}`}>
          Manage the cards in the "Unlock Property Value" section on the Services page — add,
