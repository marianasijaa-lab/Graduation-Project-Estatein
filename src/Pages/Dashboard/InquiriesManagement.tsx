...
// Every Contact field, for the detail view.
...
// Dashboard page: review, triage and delete inquiries submitted from the site.
export const InquiriesManagement = () => {
  ...
  // Inquiry-type filter options come from the data, not a fixed list.
  const typeOptions = useMemo(
    () => [
      ALL_TYPES,
      ...Array.from(new Set(contacts.map((c) => c.inquiryType).filter((t): t is string => !!t))).sort(),
    ],
    [contacts],
  );
  ...
  const filteredContacts = useMemo(() => {
    ...
    return contacts
      .filter((contact) => {
        const matchesSearch =
          fullName(contact).toLowerCase().includes(term) ||
          contact.email.toLowerCase().includes(term);
        const matchesStatus = statusFilter === ALL_STATUSES || contact.status === statusFilter;
        const matchesType = typeFilter === ALL_TYPES || contact.inquiryType === typeFilter;
  ...
  const openRowDetail = (contact: FirestoreContact) => setDetailTarget(contact);
  const handleRowKeyDown = (e: React.KeyboardEvent, contact: FirestoreContact) => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      openRowDetail(contact);
    }
  };
  ...
  const statusSelectClass = `rounded-lg border px-2.5 py-1.5 text-xs font-medium outline-none cursor-pointer transition-colors ${
    isDark
      ? "bg-bg-dark border-bg-gray-1 text-white focus:border-primary"
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <p className={`text-sm ${isDark ? "text-gray" : "text-gray-500"}`}>
          Review the messages visitors send through the Contact form and the property inquiry
          form — triage each one by status and remove spam or resolved threads.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3 lg:flex-row">
        ...
        <select
          ...
          className={`${inputClass} px-4 py-3 cursor-pointer lg:w-48`}
        >
          <option value={ALL_STATUSES} className={isDark ? "bg-bg-dark" : "bg-white"}>
            All Statuses
          </option>
        ...
        >
          {typeOptions.map((option) => (
            <option key={option} value={option} className={isDark ? "bg-bg-dark" : "bg-white"}>
              {option === ALL_TYPES ? "All Types" : option}
            </option>
          ))}
        </select>
      </div>
      ...
      {status === "failed" && (
        <div className="rounded-2xl border border-rose-500/30 bg-rose-500/10 py-16 text-center text-sm text-rose-500">
          Couldn't load inquiries. Please try again.
        </div>
      )}
            ...
            <tbody>
              {filteredContacts.map((contact) => (
                <tr
                  key={contact.id}
                  tabIndex={0}
                  aria-label={`View details for ${fullName(contact)}`}
                  onClick={() => openRowDetail(contact)}
                ...
                >
                  ...
                  <td className={`px-5 py-3 whitespace-nowrap ${isDark ? "text-gray" : "text-gray-600"}`}>
                  ...
                  <td className="px-5 py-3">
                    <select
                      ...
                      aria-label={`Status for ${fullName(contact)}`}
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        aria-label={`Delete inquiry from ${fullName(contact)}`}
          ...
          {filteredContacts.map((contact) => (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                ...
                <span className={`shrink-0 text-xs ${isDark ? "text-gray" : "text-gray-500"}`}>
                  {contact.inquiryType || "—"}
                </span>
              </div>
              ...
              <p className={`mt-2 text-xs ${isDark ? "text-gray" : "text-gray-400"}`}>
                {formatCreatedAt(contact.createdAt)}
              </p>
              ...
              <div className={`mt-4 flex items-center gap-2 pt-4 border-t ${isDark ? "border-bg-gray-1" : "border-gray-200"}`}>
                <select
                  ...
                  onChange={(e) => handleStatusChange(contact, e.target.value as ContactStatus)}
                  aria-label={`Status for ${fullName(contact)}`}
                  className={`${statusSelectClass} flex-1`}
                >
                  {STATUS_OPTIONS.map((option) => (
                    <option key={option} value={option} className={isDark ? "bg-bg-dark" : "bg-white"}>
                      {STATUS_LABEL[option]}
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-rose-500 transition-colors cursor-pointer border ${
      ...
      {detailTarget && (
        <DetailModal
          title={fullName(detailTarget)}
          fields={buildContactDetailFields(detailTarget)}
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget
            ? `The message from "${fullName(deleteTarget)}" will be permanently removed. This can't be undone.`
            : ""
        }
        confirmLabel="Delete"
