...
function validate(values: ContactInfoFormState): ContactInfoFormErrors {
  ...
  const enabledMissingUrl = values.socialLinks.some((link) => link.enabled && !link.url.trim());
  if (enabledMissingUrl) {
    errors.social = "Every enabled social link needs a URL.";
  }

  return errors;
}

// Dashboard page: edit the single site-wide contact settings document.
export const ContactInfoManagement = () => {
  ...
  // Re-seed the form from the store whenever fresh data arrives — but never
  // clobber edits the admin has already started.
  useEffect(() => {
    if (!dirty) setValues(buildState(contactInfo));
  }, [contactInfo, dirty]);
  ...
  const addSocialLink = () => {
    ...
    setField("socialLinks", [
      ...values.socialLinks,
      { platform: nextPlatform, url: "", enabled: true },
    ]);
  };

  const removeSocialLink = (index: number) => {
    setField("socialLinks", values.socialLinks.filter((_, i) => i !== index));
  };
  ...
  // Platforms still free to pick for the row at `index` (each platform once).
  const availablePlatforms = (index: number): SocialPlatform[] => {
    const usedElsewhere = new Set(
      values.socialLinks.filter((_, i) => i !== index).map((l) => l.platform),
    );
  ...
  return (
    <div className="flex flex-col gap-6">
      <p className={`text-sm ${isDark ? "text-gray" : "text-gray-500"}`}>
        These details feed the contact info and social links shown in the site footer and on the
        Contact page. Changes are saved to a single settings record.
      </p>

      {status === "failed" && (
        <div className="rounded-2xl border border-rose-500/30 bg-rose-500/10 py-4 px-5 text-center text-sm text-rose-500">
          Couldn't load the current settings — you're editing the defaults.
        </div>
      )}

      <form onSubmit={handleSubmit} className={`rounded-2xl border p-6 sm:p-8 flex flex-col gap-6 ${panelClass}`}>
        ...
        <div>
          ...
          <input
            id="ci-address"
            type="text"
            placeholder="123 Estatein Plaza, City Center, Metropolis"
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              disabled={values.socialLinks.length >= SOCIAL_PLATFORMS.length}
          ...
          <div className="flex flex-col gap-3">
            {values.socialLinks.map((link, index) => (
              ...
              >
                <select
                  value={link.platform}
                  onChange={(e) => setSocialLink(index, { platform: e.target.value as SocialPlatform })}
                  aria-label="Platform"
                  className={`px-3 py-2.5 rounded-lg border outline-none cursor-pointer transition-all sm:w-40 ${inputBgClass}`}
                >
                  {availablePlatforms(index).map((platform) => (
                    <option key={platform} value={platform} className={isDark ? "bg-bg-dark" : "bg-white"}>
                ...
                <input
                  ...
                  aria-label={`${PLATFORM_LABEL[link.platform]} URL`}
                  className={`flex-1 px-3 py-2.5 rounded-lg border outline-none transition-all ${inputBgClass}`}
                />
                <label className={`flex items-center gap-2 text-sm ${isDark ? "text-white" : "text-gray-900"}`}>
                  <input
                    type="checkbox"
                    checked={link.enabled}
                    onChange={(e) => setSocialLink(index, { enabled: e.target.checked })}
                ...
                <button
                  ...
                  aria-label={`Remove ${PLATFORM_LABEL[link.platform]}`}
        ...
        <div className={`flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-3 pt-2 border-t ${isDark ? "border-bg-gray-1" : "border-gray-200"}`}>
          {saveState === "saved" && (
            <span className="text-sm text-green-500 sm:mr-auto">Settings saved.</span>
          )}
          {saveState === "error" && (
            <span className="text-sm text-rose-500 sm:mr-auto">{saveError}</span>
          )}
          <Button
            text={saveState === "saving" ? "Saving…" : "Save Changes"}
            variant="primary"
            type="submit"
